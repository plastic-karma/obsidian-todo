...
| `invalid_input_filter` | 2 | A `/list` argument is not a `#project`, `@tag`, nonempty `!state`, or supported `due:` token, or multiple distinct due selectors are supplied; includes `field: input`. Project/tag validation and unknown configured states retain their existing error codes. |
...
A first non-whitespace token beginning with `/` MUST dispatch a command, never create a task. Supported commands are the exact lowercase tokens `/list` and `/sync`. `/list` optionally accepts whitespace-delimited `#project`, `@tag`, `!state`, and `due:` filters. `/list` MUST include all tasks, including terminal states, unless excluded by explicit filters. Projects and tags combine with AND; repeated states form an OR set. Exact duplicates MUST be deduplicated. Project/tag spelling and configured state IDs MUST use existing list validation; unknown tags may yield no matches. `/list #personal @chores !open` selects open chores in personal. Commands MUST NOT parse bare natural dates or treat bare text as a name/query. Unknown slash commands and malformed arguments MUST fail without mutation; slashes and `!state` inside ordinary task names remain literal text. Listing MUST use the complete-store validation, locking, and deterministic sort of section 13.2 in both supported store versions, without changing ordinary `otodo list` defaults.

The supported due filters are the exact lowercase tokens `due:today`, `due:tomorrow`, `due:overdue`, and `due:none`. Today and tomorrow select tasks whose `due_date` equals the submission's local current date or the next local calendar date, respectively; `--today` overrides the reference date. Calendar overflow MUST fail under existing date validation. Overdue MUST use section 13.2's semantics: nonterminal tasks with `due_date < today`. None MUST select tasks with no due date (omitted or null), not all dates. Due times MUST NOT affect these date-only predicates. A due filter MUST combine with all project, tag, and state filters using AND. Repeated identical due selectors MUST be deduplicated; multiple distinct selectors or unsupported due values MUST fail with `invalid_input_filter` (usage, `field: input`). `/list #personal @chores !open due:today` selects today's open chores in personal.
...
Suggestions MUST use existing project slugs and tags across all tasks, including terminal tasks, with case-insensitive prefix matching and original spelling on acceptance. They MUST also complete `/list` and `/sync` in the first token, configured `!state` IDs and all four `due:` filters in `/list` arguments, and `ours`/`theirs` as the first `/sync` argument. Outside history browsing, Up/Down selects available suggestions; Shift-Tab MUST select the previous suggestion in either mode. Tab accepts the whole token at the cursor without corrupting surrounding Unicode text. New saved tags MUST be immediately available; F5 explicitly refreshes the suggestion catalog from disk. Store locks MUST NOT remain held while waiting for keyboard input. Enter submits exactly the current line; failed validation retains the draft for correction. Successful `/list` commands MUST display results without incrementing the created count, and PgUp/PgDn MUST allow every result row to be reached. Successful submissions advance to the next queued draft or clear the line; list results remain visible when suggestions are not displayed until replaced by another successful command or task save. Multiline paste MUST queue drafts for individual Enter confirmation. Esc/Ctrl-C exits and discards unsaved drafts; Ctrl-D exits when all drafts are empty. Successful saves remain committed. Terminal raw mode, bracketed paste, cursor visibility and alternate-screen state MUST be restored on normal/error exits. I/O, concurrency and unsupported-generation errors MUST exit rather than invite an uncertain creation retry.

The TUI MUST keep a session-local history of the latest 100 successful nonblank input lines, including task captures and slash commands, excluding consecutive identical entries. It MUST retain the original input text, not rendered task metadata, and MUST NOT write history to disk. When no suggestions are available or history browsing is already active, Up/Down MUST recall older/newer entries without wrapping or submitting them. Down past the newest entry MUST restore the original draft and cursor. Recalled entries MUST remain editable without changing recorded history; navigation MUST preserve queued paste drafts. Recalling an entry or restoring a draft MUST enforce the combined 16 KiB input/queue bound, refusing without changing the line, cursor, history position, or queue. Every nonblank Enter attempt MUST end browsing before parsing or execution. Failed submissions MUST retain the current line and cursor as the live draft without adding it to history. Successful submission or advancing a blank line MUST end browsing and make the next queued line (or empty line) the live draft. History MUST be discarded on exit.
...
Rootless executable capabilities MUST include `task_input`, `task_due_times`, and `git_sync`. Tests MUST defend requested examples, URL/title preservation, metadata and date/time boundaries, precedence, local/DST arithmetic, Unicode completion/editing, rejected-draft preservation, partial-stream failure, both store versions, time lifecycle/clear invariants, and unchanged schema/configuration bytes. Slash-command coverage MUST defend combined list filters, all four due selectors and local calendar boundaries, terminal-state inclusion, read-only listing, unknown-command/filter failures, mixed capture/list/sync streams, complete-store validation before filtered results, and the synchronization guarantees in section 18.1.
